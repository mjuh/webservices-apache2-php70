...
Docker container with Apache PHP 7.0 build by [Nix package manager](https://nixos.org/).
...
## Building Apache PHP 7.0

To build Apache PHP 7.0, follow these steps:
...
[![Build Status](https://jenkins.majordomo.ru/job/webservices/job/apache2-php70/job/master/lastBuild/badge/icon?config=webservices-apache2-php70)](https://jenkins.majordomo.ru/job/webservices/job/apache2-php70/job/master/lastBuild/badge/icon?config=webservices-apache2-php70)

## Using Apache PHP 7.0

To use Apache PHP 7.0, call:

``` shell
docker inspect docker-registry.intr/webservices/apache2-php70 | grep cmd
```
...
docker run --read-only --network=host \
        ...
        --ulimit stack=-1:-1 \
        --cap-add SYS_ADMIN \
        --security-opt apparmor:unconfined docker-registry.intr/webservices/apache2-php70:latest
```
...
* `$SOCKET_HTTP_PORT` any free port on your computer, e.g. `8070`.

## Contributing to Apache PHP 7.0

To contribute to Apache PHP 7.0, follow these steps:
